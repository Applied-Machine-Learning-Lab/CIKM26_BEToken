"""
BE memory-token training on RoleLLM/RoleBench.

Overview
--------
- Wraps a base causal LM with a learnable memory (MemoryCell).
- Joint AE (reconstruct system prompt) + KD (assistant-prefix distillation).
- Simple evaluation that saves generated outputs (no ROUGE).
- Prompt styles supported: short, LLaMA3-style, Qwen3(ChatML-style).
- Uses RoleBench role-specific instructions for KD.
"""
...
def format_prompt(instruction: str, style: str = "short") -> str:
    if style == "short":
        ...
        # user+assistant headers (without the actual answer body)
...
class MemoryCell(torch.nn.Module):
    """
    A simple memory wrapper:
    - Freezes base model params.
    - Adds learnable memory tokens (num_mem_tokens x memory_dim).
    - Concatenates memory embeddings before inputs for both forward and generate.
    """
...
class RoleSpecificDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "response_text": response_text,        # used for KD
...
def kd_loss_on_assistant_prefix(
    tokenizer,
    teacher_model,                # base model (with system text)
    student_with_mem: MemoryCell, # memory wrapper (no system text)
    system_prompt: str,
    prompt_text: str,             # user + assistant BOS produced by format_prompt
    response_text: str,           # ground-truth answer (can be empty)
    N_mem_tokens: int,
    kd_T_tokens: int = 32,
    kd_temperature: float = 2.0,
    use_teacher_gen: bool = False,   # If True: always use teacher-generated tokens; else use dataset answer (skip KD if empty)
) -> Tuple[torch.Tensor, int]:
    """
    Returns: (KD loss, number of tokens used T')
    Teacher input  : [system + prompt + answer[:T'-1]] -> take logits over positions [prefix-1 : prefix-1+T']
    Student input  : [MEM + prompt + answer[:T'-1]]   -> take the same slice aligned after the memory and prompt.

    Args:
        use_teacher_gen:
            True  => ignore dataset answer; use the teacher to greedily generate kd_T_tokens as the forced sequence.
            False => use dataset answer; if empty, skip KD and return 0.

    KD formulation: Temperature-scaled cross-entropy between teacher soft targets and student logits.
    """
    ...
    if use_teacher_gen:
        ...
        if response_text and response_text.strip():
            ...
            # No generation and empty answer => skip KD
    ...
    # Teacher-forcing input: system + prompt + answer[:Tprime-1]
    ...
    # Student-forcing input: prompt + answer[:Tprime-1] with memory prepended via wrapper
    student_input_ids = torch.cat([prompt_ids[0], ans_ids_full[:max(0, Tprime-1)]], dim=0).unsqueeze(0)
    student_outputs, _ = student_with_mem(input_ids=student_input_ids)  # logits: (1, M+L, V)
    ...
    # Temperature KD (Hinton): T^2 * CE_soft = - T^2 * mean(sum q * log p)
    T = kd_temperature
    log_p = F.log_softmax(student_logits_slice / T, dim=-1)   # (1, T', V)
    with torch.no_grad():
        q = F.softmax(teacher_logits_slice / T, dim=-1)       # (1, T', V)
...
# ================== Single role: Phase-1 (AE + KD, multi-position KL) =================
def run_single_role(
    args,
    model,
    tokenizer,
    ...
    """
    Run Phase-1 only: AE + KD (assistant prefix, multi-position) with early stopping.
    After training, save curves and [MEM], then run Phase-3 evaluation (outputs only, no ROUGE).
    """
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    dtype = getattr(torch, args.dtype)
    ...
    if ae_vector.dim() == 1: ae_vector = ae_vector.unsqueeze(0)
    ...
    # ---------- KD training data (RoleBench instructions, for unsupervised KD only) ----------
    ...
    # ============================ PHASE 1 ============================
    print("\n" + "="*80)
    print(" PHASE 1: AE + KD on assistant prefix (multi-position, teacher-forcing)")
    print("="*80 + "\n")
    ...
    # Teacher's system text used only for AE; KD driven by instruction samples
    ...
    # Iterations: each step does one KD sample and one AE step
    ...
    for it in pbar1:
        ...
        with torch.amp.autocast(device_type='cuda' if device=='cuda' else 'cpu', dtype=dtype):
            # ----- AE loss: reconstruct system ([MEM] + [AE] + system_tokens) -----
            ...
            # ----- KD loss: first T positions after assistant start -----
            kd_loss, used_T = kd_loss_on_assistant_prefix(
                ...
                use_teacher_gen=args.kd_use_teacher_gen,  # always teacher-gen OR always dataset answer
            )
            ...
            if used_T > 0:
                ...
                # No KD available (skipped), use AE only
                loss_total = loss_ae
        ...
        if lt < best_loss - 1e-8:
            ...
            no_improve += 1
        pbar1.set_postfix({"total": lt, "AE": la, "KD": lk, "best": best_loss, "pat": no_improve})
    ...
    print("✅ Phase 1 complete. Best total loss:", best_loss, " at iter:", best_iter)

    # Save curves and [MEM]
    tag = param_tag(role_resolved, N_mem_tokens, lm_loss_weight, initial_max_iterations, 0, args.ae_vector_path)
    ...
    plt.xlabel("Iteration"); plt.ylabel("Loss"); plt.title(f"Phase 1 Losses ({tag})"); plt.legend()
    ...
    # Direct evaluation (epoch=0 marker): no ROUGE, only save outputs
    rec = evaluate_role(
        args, tokenizer, model, model_with_memory, role_resolved,
        N_mem_tokens, lm_loss_weight, initial_max_iterations,
        0, args.ae_vector_path, out_dir, results_jsonl_path,
...
# ================================== Main ====================================
def parse_arguments():
    p = argparse.ArgumentParser(description="Phase-1 only: AE + KD (assistant prefix, multi-position) and evaluation (no ROUGE)")
    # Model / Tokenizer / AE
    p.add_argument('--model_path', type=str, required=True)
    p.add_argument('--tokenizer_path', type=str, default=None)
    p.add_argument('--ae_vector_path', type=str, default="./results/ae_token_tuning2/ae_vector.pt")
    ...
    p.add_argument('--initial_lr', type=float, default=1e-1)
    ...
    p.add_argument('--kd_temperature', type=float, default=2.0, help='KD temperature (tau)')
    # If set: always use teacher-generated T tokens; otherwise use dataset answer (skip KD when answer empty).
    p.add_argument('--kd_use_teacher_gen', action='store_true',
                   help='If set, ignore dataset answers and always use teacher-generated T tokens for KD; otherwise use dataset answers.')
    ...
    p.add_argument('--num_roles', type=int, default=0, help='>0 to evaluate only the first n roles (by test.jsonl order)')
